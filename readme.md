...
* `void hm_iter_next(my_iter *i)`
  - advances iterator `i` to point to yet-unvisited element in no meaningful order or `NULL` if all elements have been visited. It is invalid to call this function if hashtable had new element inserted since corresponding `hm_iter_new` was called (modifying or removing elements is ok).
...
hashes, join discussion if you have suggestions
...
## My little dream
I dream to live and work as a programmer in Singapore. I'm young and have little
professional programming experience (C programmer on internship at Weigandt
Consulting and SQL as a billing admin in local telecom company). Path to dream is also obstructed
by the fact that im stuck in Crimea.
If you, by chance, have any advice that could help me accomplish this dream
